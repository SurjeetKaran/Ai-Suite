const Anthropic = require("@anthropic-ai/sdk");
const {
  selectKey,
  updateKeyUsage,
  markKeyFailed
} = require("../../utils/keyBalancer");
const LoadBalancerConfig = require("../../models/LoadBalancerConfig");
const log = require("../../utils/logger");

const PROVIDER = "anthropic";

async function callAnthropicProvider(prompt, model) {
  const lbConfig = await LoadBalancerConfig.findOne() || { retryCount: 1 };
  const maxRetries = lbConfig.retryCount;

  for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
    let selectedKey = null;

    try {
      selectedKey = await selectKey(PROVIDER);

      const client = new Anthropic({
        apiKey: selectedKey.key
      });

      const response = await client.messages.create({
        model,
        max_tokens: 1024,
        messages: [{ role: "user", content: prompt }]
      });

      const text = response.content?.[0]?.text || "";
      const tokensUsed =
        response.usage?.input_tokens + response.usage?.output_tokens || 0;

      await updateKeyUsage(selectedKey._id, tokensUsed);

      log("INFO", "[Anthropic] Success", { model, tokensUsed });

      return { text, tokensUsed };

    } catch (err) {
      log("ERROR", "[Anthropic] Failed", { attempt, error: err.message });

      if (selectedKey?._id) {
        await markKeyFailed(selectedKey._id, err.message);
      }

      if (attempt > maxRetries) {
        return {
          text: "Claude is temporarily unavailable.",
          tokensUsed: 0
        };
      }
    }
  }
}

module.exports = { callAnthropicProvider };
