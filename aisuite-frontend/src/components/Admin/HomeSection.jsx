...
// src/components/Admin/HomeSection.jsx
import React, { useEffect, useState } from "react";
import { useAdminDashboardStore } from "../../store/adminDashboardStore";
import { motion } from "framer-motion";
import CountUp from "react-countup";
...
export default function HomeSection() {
  ...
  const { userStats, modelStats, providerStats, apiKeys, recentQueries } = dashboardData;

  // ⬆️ Model Usage Chart Data
  const modelUsageData = Object.entries(modelStats || {}).map(([model, count]) => ({
    name: model,
    value: count,
  }));

  // ⬆️ Provider Usage Chart Data
  const providerUsageData = Object.entries(providerStats || {}).map(([provider, stats]) => ({
    provider,
    totalRequests: stats.totalRequests,
  }));

  // ⬆️ API Key Health Summary
  const keyHealthCounts = {
    healthy: apiKeys.filter((k) => k.health === "healthy").length,
    failing: apiKeys.filter((k) => k.health === "failing").length,
    cooldown: apiKeys.filter((k) => k.health === "cooldown").length,
  };
  ...
  const metrics = [
    { title: "Total Users", value: userStats.totalUsers, icon: <UsersIcon className="w-6 h-6 text-blue-400" /> },
    { title: "Pro Users", value: userStats.proUsers, icon: <StarIcon className="w-6 h-6 text-amber-400" /> },
    { title: "Revenue", value: userStats.totalRevenue, prefix: "$", icon: <CurrencyDollarIcon className="w-6 h-6 text-emerald-400" /> },
    { title: "Total Queries", value: userStats.totalQueries, icon: <ChartBarIcon className="w-6 h-6 text-purple-400" /> },
  ];
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      {/* HEADER */}
      <div className="flex justify-between bg-[#1e293b]/50 p-4 rounded-2xl border border-white/5">
        <div>
          <h2 className="text-xl font-bold text-white">Overview</h2>
          <p className="text-sm text-gray-400">Track performance, usage & system health.</p>
        </div>
        <div className="flex items-center gap-2 bg-[#0f172a] p-1 rounded-xl border border-white/10">
          <div className="flex items-center px-3 border-r border-white/10">
            <CalendarIcon className="w-4 h-4 text-gray-400 mr-2" />
            <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="bg-transparent text-gray-300 text-xs" />
          </div>
          <div className="flex items-center px-3">
            <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="bg-transparent text-gray-300 text-xs" />
          </div>
          <button onClick={fetchWithDates} className="px-4 py-1 bg-blue-600 text-white rounded-lg">Filter</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {metrics.map((m, i) => (
          <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="p-5 rounded-2xl bg-[#0f172a]/50 border border-white/5">
            <div>
              <p className="text-xs text-gray-400 uppercase">{m.title}</p>
              <h3 className="text-2xl text-white font-bold">{m.prefix}<CountUp end={m.value} duration={2} /></h3>
            </div>
            <div className="p-3 bg-white/5 rounded-xl mt-3">{m.icon}</div>
          </motion.div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          ...
          <div key={idx} className="bg-[#1e293b]/50 p-5 rounded-xl border border-white/5 flex items-center justify-between">
            <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Revenue Chart */}
        <div className="lg:col-span-2 bg-[#1e293b]/50 p-6 rounded-2xl border border-white/5">
          <h3 className="text-lg text-white font-bold mb-4">Revenue Trend</h3>
        ...
        <div className="bg-[#1e293b]/50 p-6 rounded-2xl border border-white/5">
          <h3 className="text-lg text-white font-bold mb-4">User Distribution</h3>
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie data={subscriptionSplit} dataKey="value" cx="50%" cy="50%" outerRadius={90}>
                {subscriptionSplit.map((e, i) => (
                  <Cell key={i} fill={e.name === "Pro" ? "#3b82f6" : "#10b981"} />
                ))}
              </Pie>
              <Legend />
            </PieChart>
      ...
      {/* MODEL USAGE CHART */}
      <div className="bg-[#1e293b]/50 p-6 rounded-2xl border border-white/5">
        <h3 className="text-lg text-white font-bold mb-4">Model Usage Stats</h3>
        <ResponsiveContainer width="100%" height={300}>
      ...
      {/* PROVIDER USAGE */}
      <div className="bg-[#1e293b]/50 p-6 rounded-2xl border border-white/5">
        <h3 className="text-lg text-white font-bold mb-4">Provider Usage Stats</h3>
        <ResponsiveContainer width="100%" height={300}>
                ...
                recentQueries.map((query, idx) => {
                  ...
                  return (
                    <tr key={idx} className="hover:bg-white/5 transition-colors group">
                      ...
                      <td className="p-4 text-xs font-mono text-gray-500 group-hover:text-gray-400">{formattedDate}</td>
                      <td className="p-4 text-gray-400 text-sm max-w-xs">
